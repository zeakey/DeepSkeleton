...
ImageMultLabelDataLayer<Dtype>::~ImageMultLabelDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void ImageMultLabelDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK(gt_channels == 3) << "GT image channel number should be 3";
...
template <typename Dtype>
void ImageMultLabelDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageMultLabelDataLayer<Dtype>::load_batch(LabelmapBatch<Dtype>* batch) {
...
INSTANTIATE_CLASS(ImageMultLabelDataLayer);
REGISTER_LAYER_CLASS(ImageMultLabelData);
