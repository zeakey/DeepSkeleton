...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    //LOG(ERROR) << "Forwarding " << i <<"->"+layer_names_[i];
    //if(bottom_vecs_[i].size())if(bottom_vecs_[i][0]->has_nan())LOG(WARNING)<<"layer id: "<<i<<", "+layer_names_[i]+" BOTTOM NAN!";
    
    Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]);
    
    //if(top_vecs_[i].size())if(top_vecs_[i][0]->has_nan())LOG(WARNING)<<"layer id: "<<i<<", "+layer_names_[i]+" TOP NAN!";
    loss += layer_loss;
